...
import pandas as pd
import numpy as np
inputfile = './data_chapter10/original_data.xls'  # 输入的数据文件
...
data = pd.read_excel('./data_chapter10/original_data.xls')
print('初始状态的数据形状为：', data.shape)
# 删除热水器编号、有无水流、节能模式属性
data.drop(labels=["热水器编号", "有无水流", "节能模式"], axis=1, inplace=True)
print('删除冗余属性后的数据形状为：', data.shape)
data.to_csv('./data_chapter10/water_heart.csv', index=False)


# 读取数据
data = pd.read_csv('./data_chapter10/water_heart.csv')
# 划分用水事件
...
sj.to_csv('./data_chapter10/sj.csv', index=False)
...
data = pd.read_excel('./data_chapter10/water_hearter.xlsx')  # 读取热水器使用数据记录
sj = pd.read_csv('./data_chapter10/sj.csv')  # 读取用水事件记录
# 转换时间格式
data["发生时间"] = pd.to_datetime(data["发生时间"], format="%Y%m%d%H%M%S")
...
sj_final.to_excel('./data_chapter10/sj_final.xlsx', index=False)
